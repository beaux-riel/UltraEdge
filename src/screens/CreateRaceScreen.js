...
const CreateRaceScreen = ({ route, navigation }) => {
  ...
  const [presetDistanceModalVisible, setPresetDistanceModalVisible] = useState(false);
  
  // Preset time goals
  const [usePresetTimeGoal, setUsePresetTimeGoal] = useState(false);
  const [presetTimeGoalModalVisible, setPresetTimeGoalModalVisible] = useState(false);
  ...
  const onImportantDateTimeChange = (event, selectedDateTime) => {
    ...
    if (importantDatePickerMode === "date") {
      ...
      const existingTimePart = newImportantDateTimeString.split(" ").slice(1).join(" ");
      if (existingTimePart) {
        setNewImportantDateTimeString(`${month}/${day}/${year} ${existingTimePart}`);
      } else {
