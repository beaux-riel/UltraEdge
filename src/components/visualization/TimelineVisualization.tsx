...
import { Card, IconButton, Portal, Modal, Button } from 'react-native-paper';
...
interface TimelineVisualizationProps {
  ...
  raceInfo?: RaceInfo;
  mode?: 'time' | 'distance';
  onEntryPress?: (entry: NutritionEntry | HydrationEntry & { type: 'nutrition' | 'hydration' }) => void;
}
...
const TimelineVisualization: React.FC<TimelineVisualizationProps> = ({ 
  ...
  const [selectedEntry, setSelectedEntry] = useState<(NutritionEntry | HydrationEntry) & { type: 'nutrition' | 'hydration' } | null>(null);
  ...
  // Handle entry press
  const handleEntryPress = (entry: (NutritionEntry | HydrationEntry) & { type: 'nutrition' | 'hydration' }) => {
    setSelectedEntry(entry);
    setModalVisible(true);
  };
  
  // Calculate timeline position for an entry
  const calculateTimelinePosition = (entry: NutritionEntry | HydrationEntry) => {
    if (mode === 'time') {
      ...
      return (entry.distance || 0 / (raceInfo.distance || 50)) * timelineWidth;
    }
  };
  
  // Get color for entry type
  const getEntryColor = (entry: NutritionEntry | HydrationEntry, isNutrition: boolean) => {
    if (isNutrition) {
      // Color based on nutrition type
      const nutritionEntry = entry as NutritionEntry;
      if (nutritionEntry.foodType?.toLowerCase().includes('gel')) {
        return theme.colors.primary;
      } else if (nutritionEntry.foodType?.toLowerCase().includes('bar')) {
        return theme.colors.secondary;
      } else if (nutritionEntry.foodType?.toLowerCase().includes('drink')) {
        return theme.colors.tertiary;
      }
      return theme.colors.primary;
    } else {
      // Color based on hydration type
      const hydrationEntry = entry as HydrationEntry;
      if (hydrationEntry.liquidType?.toLowerCase().includes('water')) {
        return theme.colors.blue;
      } else if (hydrationEntry.liquidType?.toLowerCase().includes('electrolyte')) {
        return theme.colors.purple;
      } else if (hydrationEntry.liquidType?.toLowerCase().includes('soda')) {
  ...
  const renderTimelineMarkers = () => {
    ...
    const totalIntervals = mode === 'time' ? totalDuration / interval : (raceInfo.distance || 50) / interval;
  ...
  const renderMilestones = () => {
    ...
    return raceInfo.milestones.map((milestone, index) => {
      const position = mode === 'time'
        ? ((milestone.time || 0) / (raceInfo.duration || 720)) * timelineWidth
        : ((milestone.distance || 0) / (raceInfo.distance || 50)) * timelineWidth;
      ...
      <Portal>
        ...
        >
          <Card>
            <Card.Title 
              title={isNutrition 
                ? (selectedEntry as NutritionEntry).foodType 
                : (selectedEntry as HydrationEntry).liquidType
              }
            ...
            <Card.Content>
              {isNutrition ? (
                <View>
                  <Text style={styles.detailLabel}>Calories: {(selectedEntry as NutritionEntry).calories || 0}</Text>
                  <Text style={styles.detailLabel}>Carbs: {(selectedEntry as NutritionEntry).carbs || 0}g</Text>
                  <Text style={styles.detailLabel}>Protein: {(selectedEntry as NutritionEntry).protein || 0}g</Text>
                  <Text style={styles.detailLabel}>Fat: {(selectedEntry as NutritionEntry).fat || 0}g</Text>
                  ...
                  {(selectedEntry as NutritionEntry).notes && (
                    <Text style={styles.detailLabel}>Notes: {(selectedEntry as NutritionEntry).notes}</Text>
                  )}
                </View>
              ) : (
                <View>
                  <Text style={styles.detailLabel}>Volume: {(selectedEntry as HydrationEntry).volume || 0} {(selectedEntry as HydrationEntry).volumeUnit || 'ml'}</Text>
                  <Text style={styles.detailLabel}>Timing: {selectedEntry.timing || 'N/A'}</Text>
                  <Text style={styles.detailLabel}>Frequency: {selectedEntry.frequency || 'N/A'}</Text>
                  {(selectedEntry as HydrationEntry).consumptionRate && (
                    <Text style={styles.detailLabel}>Consumption Rate: {(selectedEntry as HydrationEntry).consumptionRate} ml/hr</Text>
                  )}
                  {(selectedEntry as HydrationEntry).temperature && (
                    <Text style={styles.detailLabel}>Temperature: {(selectedEntry as HydrationEntry).temperature}</Text>
                  )}
                </View>
              )}
            </Card.Content>
