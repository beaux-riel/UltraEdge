/**
 * UltraEdge Create Checkpoint Screen
 * Form to add a new checkpoint/aid station to an event
 */
...
import { CheckpointType, CheckpointInsert } from '../../lib/database.types';
...
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'CreateCheckpoint'>;
type ScreenRouteProp = RouteProp<RootStackParamList, 'CreateCheckpoint'>;
...
export default function CreateCheckpointScreen() {
  ...
  const [useMetric, setUseMetric] = useState(false); // false = imperial (mi/ft), true = metric (km/m)
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const checkpointData: Omit<CheckpointInsert, 'event_id' | 'order_index'> = {
        name: name.trim(),
      ...
      Alert.alert('Error', 'Failed to create checkpoint. Please try again.');
    } finally {
      setSaving(false);
    }
  }, [
    ...
    hasPacerDropoff, selectedSupplies, notes, eventId, addCheckpoint, navigation
  ]);
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <BodySmall color="secondary" style={{ marginTop: spacing.xs }}>
            Define a point along your race course
          </BodySmall>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.unitToggleRow}>
            <Label>UNITS</Label>
            <View style={[styles.unitToggle, { backgroundColor: colors.cream, borderColor: colors.border }]}>
              <TouchableOpacity
                style={[
                  styles.unitOption,
        ...
        <View style={styles.row}>
          <View style={[styles.section, { flex: 1 }]}>
            <Label style={{ marginBottom: spacing.sm }}>DISTANCE ({useMetric ? 'km' : 'mi'})</Label>
            <TextInput
              style={[
                styles.input,
                {
          ...
          <View style={[styles.section, { flex: 1 }]}>
            <Label style={{ marginBottom: spacing.sm }}>ELEVATION ({useMetric ? 'm' : 'ft'})</Label>
            <TextInput
              style={[
                styles.input,
                {
