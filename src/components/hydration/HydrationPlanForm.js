...
import HydrationEntryForm from './HydrationEntryForm';

/**
 * Form for creating or editing a hydration plan
 ...
 */
const HydrationPlanForm = ({ plan = null, onSave, onCancel, isEditing = false }) => {
  const { theme, isDarkMode } = useAppTheme();
  const { getHydrationPlanTemplates, createPlanFromTemplate } = useNutritionHydration();
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      const result = await getHydrationPlanTemplates();
      if (result.success) {
  ...
  const handleSelectTemplate = async (templateId) => {
    const result = await createPlanFromTemplate(templateId, 'hydration');
    if (result.success) {
  ...
  return (
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Title title={isEditing ? 'Edit Hydration Plan' : 'Create Hydration Plan'} />
        <Card.Content>
          <FormField
            label="Plan Name"
            value={formData.name}
            onChangeText={(value) => updateFormData('name', value)}
            placeholder="e.g., My 50K Hydration Plan"
            required
            error={errors.name}
          />
...
export default HydrationPlanForm;